...
function MapMarker(name, icon, cordX, cordY, map){
	...
	this.generateMarker = function(){
		output = '<div class="KBmap__marker" data-marker-name="'+this.name+'" style="left: '+this.cordX+'%; top: '+this.cordY+'%"><div class="marker"> </div></div>'
...
function MarkerModal(modalTitle, content, linkedMapMarker){
	...
	this.generateModal = function(){
		...
		output = ' <div  class="KBmap__markerContent"> <div class="KBmap__markerClose"><i class="fa fa-times" aria-hidden="true"></i>' +
			...
			// +
			// '<h5 class="KBmap__markerTitle">' + this.title + '</h5>';
	...
	this.clearPosition = function(){
		...
		$positionedElem.css({
			...
			'bottom': '-2000%', //self.positionedElemOffsetY,
...
jQuery( document ).ready(function() {
	...
	jQuery('body').on('click touchstart', '.KBmap__markerClose', function(event){
	...
	jQuery('body').on('click touchstart', '.KBmap__markerContent', function(){
...
function createKBmap(name, mapsrc, mapDataJSON){
	...
	var output = '<div class="KBmap__mapContainer"><div class="KBmap__mapHolder"><img src="' + mapImg + '" alt="mapa" style="width: 100%" ></div></div>';
...
function onclicckline() {

	const modalWindow = document.querySelector('.KBmap__mapHolder'),
		modalWindowOffsetTop = modalWindow.offsetTop,

		modalWindowOffsetLeft = modalWindow.offsetLeft,
		markerPoints = document.querySelectorAll('.KBmap__marker');
	console.log('fine');

	for (let i = 0; i < markerPoints.length; i++) {
		markerPoints[i].addEventListener('click', function () {
			showLine(this);
		});
	}

	const showLine = function (element) {
		const markerWidth = element.offsetWidth,
			markerHeight = element.offsetHeight,
			markerOffsetTop = element.offsetTop,
			markerOffsetLeft = element.offsetLeft,
			lineElement = document.querySelector('.line'),
			newlineElement = document.createElement("div"),
			lineWidth = modalWindowOffsetLeft - markerOffsetLeft;
		...
		document.querySelector('.KBmap__mapHolder').append(newlineElement);
